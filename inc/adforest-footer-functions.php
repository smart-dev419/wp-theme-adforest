...
if (!function_exists('adforest_header_fav_ads_callback')) {
    ...
    function adforest_header_fav_ads_callback() {
        ...
        if ($profile_type == 'notifications') {
            ?>
            <script>
                jQuery(document).ready(function () {
                    var adforest_ajax_url = jQuery('#adforest_ajax_url').val();
                    var noti_id = <?php echo $post_ad_id ?>;

                    if(noti_id > 0){
                        jQuery.post(adforest_ajax_url, {action: 'sb_read_notification', async: false, noti_id: <?php echo $post_ad_id ?>, status: "read"}).done(function (response){
                        });
                    }

                    jQuery('#sb_dy_notifications').click();
                    var uri = window.location.toString();
                    if (uri.indexOf("?") > 0) {
                        var clean_uri = uri.substring(0, uri.indexOf("?"));
                        window.history.replaceState({}, document.title, clean_uri);
    ...
    function adforest_ads_rating_reaction_emoji() {
        ...
        if ($reaction_id && $comment_id) {
            if (get_user_meta(get_current_user_id(), 'adforest_listing_review_submit_id' . $comment_id, true) == $comment_id) {
                ...
                sa_update_notification($listing_id, get_current_user_id(), $get_comment->user_id, "10");
            }
        }
        die();
    }
