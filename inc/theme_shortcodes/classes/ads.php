...
';
		...
		function adforest_get_notifications_list() {
			global $adforest_theme, $wpdb, $notification_text;     

			$table_name = $wpdb->prefix . "notification";

			$wpdb->query("UPDATE ".$table_name." SET state = '1' WHERE uid = '".get_current_user_id()."' AND type < 3");

			$notes = $wpdb->get_results("SELECT * FROM ".$table_name." WHERE uid = '".get_current_user_id()."' AND type > 1 ORDER BY created_date DESC");

			$notif_html = '<div class="row">
									<div class="col-md-12">
										<div class="noti_items_wrapper">
											<div class="noti_items_title">'. __('Notifications', 'adforest').'</div>
											<div class="noti_items_list">';
			if(count($notes) > 0){
				foreach ($notes as $note) {
					$content = "";

					if($note->type == '2'){
					   $link = '<a href="'.get_the_permalink($note->fid).'">'.get_the_title($note->fid).'</a>';
					   $content = sprintf($notification_text[$note->type - 1], $link);
					}else if($note->type == '3'){
					   $user_info = get_userdata($note->tid);
					   $link1 = '<a href="'.adforest_set_url_param(get_author_posts_url($note->tid), 'type', 'ads').'">'.$user_info->display_name.'</a>';
					   $link2 = '<a href="'.get_the_permalink($note->fid).'">'.get_the_title($note->fid).'</a>';
					...
					if($note->state == '0'){
						$notif_html .= '<div class="noti_items active" id="noti_'.$note->id.'">';
					}else{
						$notif_html .= '<div class="noti_items">';
					}

					$notif_html .= $content.'<div class="noti_action_btn">';
					if($note->state == '0'){
						$notif_html .= '<a class="noti_read_btn" data-id="'.$note->id.'"><i class="fa fa-envelope"></i></a>';
						$notif_html .= '<a class="noti_unread_btn hide" data-id="'.$note->id.'"><i class="fa fa-envelope-open"></i></a>';
					}else{
						$notif_html .= '<a class="noti_read_btn hide" data-id="'.$note->id.'"><i class="fa fa-envelope"></i></a>';
						$notif_html .= '<a class="noti_unread_btn" data-id="'.$note->id.'"><i class="fa fa-envelope-open"></i></a>';
					}
					$notif_html .= '<a class="noti_del_btn" data-id="'.$note->id.'"><i class="fa fa-trash"></i></a></div></div>';
				}
			}
	...
	' . get_post_meta($pid, '_adforest_ad_condition', true) . '
		  ...
		  <span>' . __('Featured', 'adforest') . '</span>
			...
			$output .= '</div></div><div class="col-md-9 col-sm-7 col-xs-12"><div class="row"><div class="content-area">';
			...
			$sa_custom_footer .= '<div class="sa_author">
				...
				<div class="sa-popup">
					...
					<main>
						...
						<h5 class="name">'.get_the_author_meta('display_name', $author_id).'</h5>

						<p class="since">inscrit(e) depuis le 6 octobre 2012</p>

						<div class="seller-badges">'.$badges_html.'</div>

						<div class="links">

							<a href="#"><i class="fa fa-envelope"></i> Envoyer MP</a>
					...
					<footer>
						...
						<ul>
							...
							<li><i class="fa fa-bar-chart"></i> 232&nbsp;<span>en moyenne depuis 1 an</span></li>

							<li><i class="fa fa-comments"></i> '.$au_count.'&nbsp;<span>commentaires</span></li>

							<li><i class="fa fa-thumbs-o-up"></i> 39&nbsp;<span>mentions j\'aime</span></li>
			...
			if ($sa_adtype === 'coupon') {
				...
				$sa_custom_footer .= '<a href="'.esc_url(get_the_permalink() . '#ad-rating').'" class="btn btn-block btn-outline btn-default"><i class="fa fa-comments"></i>&nbsp;<span>'.get_comments_number($pid).'</span></a>';
			...
			$sa_custom_meta .= '<li class="sa_price">'.$price.'</li>';

			$sa_custom_meta .= '<li class="sa_cat">Bons plans '.$cats_html.'</li>';
			...
			$output .=  $sa_custom_header . '<h3><a href="' . get_the_permalink() . '">' . get_the_title() . '</a></h3>' . $sa_custom_meta . $sa_custom_coupon . '<ul class="additional-info pull-right"><li></li></ul><div class="ad-details"><p>' . adforest_words_count(get_the_excerpt(), 150) . '</p>'.$sa_custom_footer.'</div></div></div></div></div></li>';
