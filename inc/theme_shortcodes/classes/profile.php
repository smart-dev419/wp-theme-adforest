...
function sa_message_popup ($uid) {

	ob_start();

	global $adforest_theme;
	$deal_post_id = sanitize_text_field( $_GET['deal_post_id'] );

?>

<div class="modal fade price-quote" id="messageModal" tabindex="-1" role="dialog" aria-hidden="true">
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            <span class="modal-title" id="lineModalLabel"><?php echo esc_html__('Contact','adforest'); ?>
        ...
        <div class="modal-body">
            ...
            <form id="send_message_pop" >
                ...
                <div class="form-group  col-md-12  col-sm-12">
                 ...
                 echo  adforest_returnEcho($attachment_html);
             }  ?>
                    </div>

                <div class="clearfix"></div>
                <div class="col-md-12  col-sm-12 margin-bottom-20 margin-top-20">
                    <input type="hidden" name="ad_post_id"       id="ad_post_id" value="<?php echo esc_attr($deal_post_id);?>" />
                    <input type="hidden" name="usr_id"           id="user_id" value="<?php echo get_current_user_id();?>" />
                    <input type="hidden" name="msg_receiver_id"  id="msg_receiver_id" value="<?php echo esc_attr($uid);?>" />
...
if (!class_exists('adforest_profile')) {
	...
	class adforest_profile {
		...
		function set_user_info($author_id) {
		...
		function adforest_profile_full_top() {
			...
			if ($adforest_theme['communication_mode'] == 'both' || $adforest_theme['communication_mode'] == 'message') {
				...
				$msgs = '<li><a href="javascript:void(0);" id="sb_dy_messages" class="menu-name" sb_action="my_msgs">' . __('Messages', 'adforest') . '</a></li>';

			}

			$notifs = '';

			if ($adforest_theme['communication_mode'] == 'both' || $adforest_theme['communication_mode'] == 'notification') {

				$notifs = '<li><a href="javascript:void(0);" id="sb_dy_notifications" class="menu-name" sb_action="my_notifs">' . __('Notifications', 'adforest') . '</a></li>';
			...
			if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
				...
				$order_history = '<li><a href="javascript:void(0);" class="menu-name" sb_action="my_orders">' . __('Package history', 'adforest') . '</a></li>';
			...
			<div class="rating">';
				...
				$got  =  count(adforest_get_all_ratings($this->user_info->ID));
	...
	' . get_user_meta($this->user_info->ID, '_sb_badge_text', true) . '</span>';
			...
			$discussion_count = adforest_get_all_discussions($author_id);
		...
		<div class="row">
			...
			<div class="container">
				...
				<div id="filters">
					...
					<li><a href="javascript:void(0);" class="menu-name" sb_action="get_profile" sb_author="<?php echo $this->user_info->ID ?>">Aperçu</a></li>

 	

					<?php if ($current_user->ID == $author->ID ){ ?>

						 <button class="dropdown-toggle" type="button" data-toggle="dropdown"><?php _e('My Ads', 'adforest') ?>
                            ...
                            <ul class="dropdown-menu">
                              ...
                              <li><a href="javascript:void(0);"><div class="menu-name sub-list-item" sb_action="my_ads"><?php _e('My Ads', 'adforest') ?></div></a></li>

                              <li><a href="javascript:void(0);"><div class="menu-name sub-list-item" sb_action="my_feature_ads"><?php _e('Featured Ads', 'adforest') ?></div></a></li>

                              <li><a href="javascript:void(0);"><div class="menu-name sub-list-item" sb_action="my_rejected_ads"><?php _e('Rejected Ads', 'adforest') ?></div></a></li>

                              <li><a href="javascript:void(0);"><div class="menu-name sub-list-item" sb_action="my_inactive_ads"><?php _e('Inactive Ads', 'adforest') ?></div></a></li>

                              <li><a href="javascript:void(0);"><div class="menu-name sub-list-item" sb_action="my_expire_sold_ads"><?php _e('Expired / Sold Ads', 'adforest') ?></div></a></li>

                              <li><a href="javascript:void(0);"><div id="adforest-fav-ads" class="menu-name sub-list-item" sb_action="my_fav_ads"><?php _e('Fav Ads', 'adforest') ?></div></a></li>
					...
					<li><a href="/author/<?php echo $author->user_login; ?>/?type=settings&tab=profile" class="menu-name">Settings</a></li>
					...
					<li><a href="javascript:void(0);" class="menu-name sb_ajax_call" sb_action="my_ads01" sb_author="<?php echo $this->user_info->ID ?>">Deals (<?php echo $ad_count; ?>)</a></li>



					<li><a href="javascript:void(0);" class="menu-name sb_ajax_call" sb_action="my_discussions" sb_author="<?php echo $this->user_info->ID ?>">Discussions (<?php echo $discussion_count ?>)</a></li>
...
<?php
		...
		function adforest_profile_full_body() {
			...
			if (isset($_GET['sb_action']) && isset($_GET['ad_id']) && isset($_GET['uid']) && $_GET['sb_action'] == 'sb_load_messages') {
				...
				$script = "<script>	jQuery(document).ready(function($){ adforest_select_msg('$_GET[ad_id]', '$_GET[uid]', 'no','$nonce_create'); }); </script>";

				$ads = new ads();

				$content = '<section class="seller-public-profile"><div class="container"><div id="adforest_res">' . $ads->adforest_load_messages($_GET['ad_id']) . '</div></section></div>' . $script . '';

			} else if (isset($_GET['sb_action']) && isset($_GET['ad_id']) && isset($_GET['uid']) && isset($_GET['user_id']) && $_GET['sb_action'] == 'sb_get_messages') {

				$script = "<script>	jQuery(document).ready(function($){ adforest_select_msg('$_GET[ad_id]', '$_GET[uid]', 'yes'," . wp_create_nonce('sb_msg_secure') . "); });</script>";

				$ads = new ads();

				$content = '<section class="seller-public-profile"><div class="container"><div id="adforest_res">' . $ads->adforest_get_messages($_GET['user_id']) . '</div></div></section>' . $script . '';

			} else if (isset($_GET['sb_action']) && $_GET['sb_action'] == 'my_ads') {

				$content = '<div id="adforest_res">' . $this->adforest_profile_get() . '</div>';

			} else if (isset($_GET['sb_action']) && $_GET['sb_action'] == 'my_ads01') {

				$content = '<div id="adforest_res">' . $this->adforest_profile_get() . '</div>';

			} else if (isset($_GET['sb_action']) && $_GET['sb_action'] == 'my_discussions') {
				...
				$content = '<section class="seller-public-profile hillaw"><div id="adforest_res">' . $this->adforest_profile_get() . '</div></section>';
		...
		<div class="col-lg-3">
			...
			<div class="sa_profile-stat">
				...
				<ul>
					...
					<li><i class="fa fa-comments"></i><?php echo $discussion_count ?> <span>discussion</span></li>
				...
				<ul>
					...
					<li><i class="fa fa-thumbs-o-up"></i><?php echo $up_count ?> <span>mentions j\'aime</span></li>
		...
		<div class="col-lg-9">
			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function(){
					...
					jQuery('.tabs-nav > button').on('click', function(e) {

						let targetBtn = jQuery(this);
						...
						if (!jQuery(this).hasClass('active')) {
							...
							jQuery('.tabs-nav > button').removeClass('active');
...
<?php
		...
		function adforest_profile_update_form() {
			...
			$email_name    =   "user_email";

			$readonly      =   "";

			if(isset($this->user_info->user_email) && $this->user_info->user_email != "" )   {   
			...
			return $counteries_load . adforest_get_location('adforest_location') . '
					...
					<div class="row">
					   ...
					   <div class="col-md-6 col-sm-6 col-xs-12">
						  ...
						  <input type="text" class="form-control margin-bottom-20" value="' . esc_attr($this->user_info->user_email) . '" '.$readonly.' name =  "'.$email_name.'">
        ...
        function adforest_my_ads($args, $paged, $show_pagination, $fav_ads) {
         ...
         // Get Normal users Ads

        function adforest_my_ads01($args, $paged, $show_pagination, $fav_ads) {

            $ads = new ads();

             return sa_the_ads('author', false, false, 'ad_post', $this->user_info->ID);

        }

         // Get Normal users discussions

        function adforest_my_discussions($args, $paged, $show_pagination, $fav_ads) {

            $ads = new ads();

             return sa_the_ads('author', false, false, 'discussions', $this->user_info->ID);
...
if (!function_exists('adforest_packages')) {
	...
	function adforest_packages() {
		...
		echo "<div class='container'>".adforest_returnEcho($package->adforest_get_packages_style_1($args, 4))."</div>";
...
if (!function_exists('adforest_load_messages')) {
	...
	function adforest_load_messages() {
		...
		echo "<div class='container'>".adforest_returnEcho($ads->adforest_load_messages($ad_id))."</div>";
...
if (!function_exists('adforest_my_msgs')) {
	...
	function adforest_my_msgs() {
		...
		echo "<div class='container'>".adforest_returnEcho($ads->adforest_get_messages($profile->user_info->ID)) . "</div>";
...
add_action('wp_ajax_my_notifs', 'adforest_my_notifs');

if (!function_exists('adforest_my_notifs')) {

	function adforest_my_notifs() {

		$profile = new adforest_profile();

		$ads = new ads();

		echo "<div class='container'>".adforest_returnEcho($ads->adforest_get_notifications_list()) . "</div>";
...
if (!function_exists('adforest_all_blocked_users_callback')) {
	...
	function adforest_all_blocked_users_callback() {
		...
		echo "<div class='container'>".adforest_returnEcho($ads->adforest_all_blocked_users_callback($profile->user_info->ID)). "</div>";
...
if (!function_exists('adforest_my_feature_ads')) {
	...
	function adforest_my_feature_ads() {
		...
		echo "<div class='container'>".adforest_returnEcho($ads->adforest_get_featured_ads_grid($args, $paged, $show_pagination, $fav_ads))."</div>";
...
if (!function_exists('adforest_received_msgs_ads_list')) {
	...
	function adforest_received_msgs_ads_list() {
		...
		echo "<div class='container'>".adforest_returnEcho($ads->adforest_get_user_ads_list())."</div>";
...
// Ajax handler for Normal users Ads

add_action('wp_ajax_my_ads01', 'adforest_my_ads01');

add_action('wp_ajax_nopriv_my_ads01', 'adforest_my_ads01');

if (!function_exists('adforest_my_ads01')) {



	function adforest_my_ads01() {

		$profile = new adforest_profile();

		$profile->set_user_info(isset($_POST['author']) && $_POST['author'] != '' ? $_POST['author'] : 0);
		...
		echo adforest_returnEcho($profile->adforest_my_ads01($args, $paged, $show_pagination, $fav_ads));
...
add_action('wp_ajax_my_discussions', 'adforest_my_discussions');

add_action('wp_ajax_nopriv_my_discussions', 'adforest_my_discussions');

if (!function_exists('adforest_my_discussions')) {



	function adforest_my_discussions() {
		...
		echo adforest_returnEcho($profile->adforest_my_discussions($args, $paged, $show_pagination, $fav_ads));
...
if (!function_exists('adforest_order_history')) {
	...
	<table class="table table-striped">
     ...
     <tbody>';
		...
		echo '<div class="container"><div class="profile-section margin-bottom-20">
...
add_action('wp_ajax_nopriv_get_profile', 'adforest_profile_get_ajax');
...
if (!function_exists('adforest_get_all_discussions')) {



	function adforest_get_all_discussions($user_id) {

		global $wpdb;

		$total = $wpdb->get_var("SELECT COUNT(*) AS total FROM  $wpdb->posts WHERE post_type = 'discussions' AND post_status = 'publish' AND post_author = '$user_id'");
...
if (!function_exists('adforest_sb_fav_ad')) {
	...
	function adforest_sb_fav_ad() {
		...
		if (get_user_meta(get_current_user_id(), '_sb_fav_id_' . $ad_id, true) == $ad_id) {
			...
			sa_update_notification($ad_id, get_current_user_id(), $author_id, "2");
...
if (!function_exists('adforest_sb_update_ad_status')) {
	...
	function adforest_sb_update_ad_status() {
		...
		if($status == "active"){
			sa_update_notification($ad_id, get_current_user_id(), $author_id, "8");
		}else if($status == "expired"){
			sa_update_notification($ad_id, get_current_user_id(), $author_id, "7");
		}
...
add_action('wp_ajax_sb_check_notification', 'adforest_check_notification');

if (!function_exists('adforest_check_notification')) {
	function adforest_check_notification() {
		adforest_authenticate_check();
		$user_id = get_current_user_id();
		...
		if($browser_notif_heats){
			$table_name = $wpdb->prefix . "notification";

			$unread_notifs = $wpdb->get_var("SELECT COUNT(id) FROM ".$table_name." WHERE uid = '".$user_id."' AND state = 0 AND type = 2");
		}

		if($browser_notif_message){
			$unread_msgs = $wpdb->get_var("SELECT COUNT(id) FROM ".$table_name." WHERE uid = '".$user_id."' AND state = 0 AND type = 1");
		}

		$unread_count = $unread_notifs + $unread_msgs;
...
add_action('wp_ajax_sb_read_notification', 'adforest_sb_read_notification');

if (!function_exists('adforest_sb_read_notification')) {
	function adforest_sb_read_notification() {
		adforest_authenticate_check();
		global $adforest_theme, $wpdb;
		...
		if($status == "read"){
			$notes = $wpdb->query("UPDATE ".$table_name." SET state = '1' WHERE id = '".$noti_id."'");

			echo '1|' . __("Updated successfully.", 'adforest');
		}else if($status == "unread"){
			$notes = $wpdb->query("UPDATE ".$table_name." SET state = '0' WHERE id = '".$noti_id."'");

			echo '1|' . __("Updated successfully.", 'adforest');
		}else if($status == "delete"){
			$notes = $wpdb->query("DELETE FROM ".$table_name." WHERE id = '".$noti_id."'");
