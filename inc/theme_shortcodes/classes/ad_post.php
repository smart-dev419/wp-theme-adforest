...
if (!function_exists('adforest_ad_posting')) {
    ...
    function adforest_ad_posting() {
        ...
        $sa_adurl = isset($params['ad_url']) && $params['ad_url'] != '' ? $params['ad_url'] : '';
		$sa_adcoupon = isset($params['ad_coupon']) && $params['ad_coupon'] != '' ? $params['ad_coupon'] : '';
		$sa_adtype = isset($params['ad_type']) && $params['ad_type'] != '' ? $params['ad_type'] : '';
		$sa_addiscount = isset($params['ad_discount']) && $params['ad_discount'] != '' ? $params['ad_discount'] : '';
        ...
        update_post_meta($pid, 'sa_adurl', ( esc_url_raw( $sa_adurl ) ) );
		update_post_meta($pid, 'sa_adtype', sanitize_text_field( $sa_adtype ) );
		update_post_meta($pid, 'sa_adcoupon', sanitize_text_field( $sa_adcoupon ) );
		update_post_meta($pid, 'sa_addiscount', sanitize_text_field( $sa_addiscount ) );
        ...
        $followers_meta = get_user_meta(get_current_user_id(), 'sa_followers', true);
        $followers = explode(',', $followers_meta);
        if(!empty($followers)){
            foreach($followers as $follow){
                sa_update_notification($pid, get_current_user_id(), $follow, "13");
            }
        }
        ...
        if (get_current_user_id() == "") {
            ...
            if (isset($adforest_theme['sb_update_rating']) && $adforest_theme['sb_update_rating']) {
                ...
                if (count($comment_exist) > 0) {
                    ...
                    $author_id = get_post_field( 'post_author', $params['ad_id'] );
                    sa_update_notification($params['ad_id'], $sender_id, $author_id, "4");

                    $rows = $wpdb->get_results("SELECT user_id FROM $wpdb->usermeta WHERE meta_key = '_sb_fav_id_".$params['ad_id']."'");

                    if (count($rows) > 0) {
                        foreach ($rows as $row) {
                            sa_update_notification($params['ad_id'], $sender_id, $row->user_id, "9");
                        }
                    }
